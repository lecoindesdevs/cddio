...
use serenity::{builder::{CreateApplicationCommand, CreateApplicationCommandOption}, model::{interactions::application_command::ApplicationCommandOptionType, Permissions}};
use super::command_parser::{self as cmd, Named};

pub fn register_root_with_perm(node: &cmd::Node, permission: Permissions) -> Vec<CreateApplicationCommand> {
    let group_iter = node.groups.list()
        .map(|group|{
            let mut app_cmd = CreateApplicationCommand::default();
            app_cmd.name(group.name());
            app_cmd.default_member_permissions(permission);
